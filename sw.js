/* Crema service worker
   ...
   - artwork is cache-first with no revalidation (fixed names, never
     change) */
/* The cache name and the precache list are GENERATED — see
   platform/gen-sw-assets.mjs, and run it after adding a file under src/
   or assets/. Both used to be kept by hand, and both are places where a
   mistake is silent and sticky: a file missing from ASSETS is never
   precached and never revalidated, and a cache name that did not change
   leaves every existing install answering out of the previous deploy's
   cache. CI fails a pull request whose sw.js is out of date.

   C is now a content hash of every file this worker can end up holding
   — the ones ASSETS fetches up front AND the ones the fetch handler
   picks up on demand — so it changes exactly when any of them do. That
   is also still the lever for evicting a bad cache — activate() purges
   every cache that is not C — but it is pulled automatically rather
   than remembered.

   ASSETS is deliberately NOT everything the app is made of. It is what
   opening the app needs: the document, the stylesheet, every ES module,
   the icons. assets/ artwork is left out on purpose — measured at
   856 KB of a 1.76 MB all-or-nothing addAll() on a first visit,
   competing with the very paint the visitor is waiting for, for images
   that appear on a bean detail page if they appear at all. It reaches
   this same cache through the cache-first branch below the first time
   it is asked for, and is offline from then on.

   './' rather than './index.html': that is the URL a navigation
   actually asks for, the manifest's start_url is "." and the TWA
   launches the same address. Precaching './index.html' alone would have
   left the one request that matters uncached on a first install. */
/* @generated by platform/gen-sw-assets.mjs — do not edit by hand */
const C = 'crema-fec39dd1b425';
const ASSETS = [
  './',
  ...
  './src/data/world.js',
  './src/domain/art.js',
  './src/domain/framing.js',
  './src/domain/premium.js',
