...
/* ---------- columns a hand-run migration hasn't added yet ----------
   ...
   the feature that needed the column.

   So a query that names such a column goes through here: the first 42703
   drops the optional columns for the rest of the session and retries
   once. The feature quietly turns itself off, the screen stays up, and
   the next load after the migration picks the column back up on its own.

   `build(has)` is called with a predicate: `has('edited_at')` is false
   once that column is known to be missing, and the request it returns
   must then not mention it — in the select for a read, in the body for a
   write, since a PATCH naming a missing column fails the same way. It
   returns either a path or a `{path, method, body, …}` descriptor.

   Columns are given up one at a time, named by the error, so an unrun
   migration only costs the feature that needed it and never a neighbour's
   column that happens to share the query. */
export function optionalColumns(names){
  const gone = new Set();
  ...
  return {
    has,
    async run(build, opts){
      const send = () => {
        const r = build(has);
        return typeof r==='string' ? rest(r, opts) : rest(r.path, r);
      };
      /* one attempt, plus at most one retry per column we can give up */
